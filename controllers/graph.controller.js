...
const FoodGraph = async (req, res) => {
  ...
  try {
    if (type === "weekly" && date) {
      await graphService.getWeeklyFoodData(userId, date, (err, data) => {
        if (err) {
          return res
            .status(500)
            .json({ error: "음식 기록 데이터 조회 중 오류 발생:" });
        }
        res.status(200).json(data);
      });
    }
  } catch (error) {
    console.error("음식 기록 데이터 조회 중 오류 발생:", error);
    return res.status(500).json({ message: "서버 내부 오류" });
  }
};
module.exports = { ExerciseGraph, WeightGraph, FoodGraph };
